...
События могут быть вызваны пользователем (клики мышью, нажатия клавиш, отправка форм) или происходить автоматически (загрузка страницы, изменение размера окна).
...
### Способ 1. `addEventListener` - основной способ работать с событиями
...
### Способ 2. `onclick` - устаревший, но встречается
