...
В этой лекции мы делаем интерфейс живым и “помнящим” пользователя: добавляем сохранение состояния, работу с `URL` и синхронизацию между вкладками.

## Практика

1. Реализуйте переключатель темы для `Storefront`:
   - `light` / `dark`;
   - класс на `body`;
   - сохранение темы в `localStorage`;
   - восстановление темы при загрузке страницы.

2. Сохраните в `localStorage` пользовательское состояние каталога:
   - `favoriteIds`;
   - содержимое корзины;
   - выбранную сортировку.

   Если сохраняете массивы или объекты, используйте `JSON.stringify()` / `JSON.parse()`.

3. Привяжите фильтры каталога к `URLSearchParams`.
   При изменении фильтров обновляйте строку запроса:
   - `search`
   - `category`
   - `stock`
   - `sort`

4. Реализуйте восстановление фильтров из `location.search`:
   - если в URL есть `search`, подставьте его в форму;
   - если в URL есть `category`, выберите нужную опцию;
   - после восстановления сразу перерисуйте каталог.

5. Добавьте сообщение `Saved`.
   Оно должно:
   - показываться после сохранения фильтров или темы;
   - исчезать через `setTimeout`;
   - перезапускать таймер при повторном сохранении.

6. Сделайте простое переключение разделов через `location.hash`:
   - `#catalog`
   - `#favorites`
   - `#settings`

   При смене хэша показывайте соответствующий блок, не перезагружая страницу.

7. Добавьте синхронизацию между вкладками через `storage`:
   - изменение темы в одной вкладке должно обновлять тему в другой;
   - изменение списка избранного тоже должно отражаться во второй вкладке.

## Домашняя работа

Соберите страницу **`Storefront - настройки и сохранение состояния`**.

### Что должно сохраняться

1. Тема оформления.
2. Выбранный язык интерфейса.
3. Текущие фильтры каталога.
4. Избранные товары.
5. Корзина.
...
1. Хранение в `localStorage`:
   - простые значения как строки;
   - массивы/объекты только через `JSON.stringify()`.

2. Восстановление состояния при загрузке:
   - применить тему;
   - восстановить язык;
   - подставить фильтры в форму;
   - восстановить избранное и корзину;
   - сразу отрисовать интерфейс в правильном состоянии.
...
4. Простая навигация по хэшу:
   - `#catalog`
   - `#favorites`
   - `#settings`
   - если хэш отсутствует, по умолчанию открывать `#catalog`.

5. Кнопка `Reset app state`:
   - очищает сохранённые ключи;
   - возвращает значения по умолчанию;
   - обновляет интерфейс без перезагрузки.

6. Событие `storage`:
   - тема и избранное должны синхронизироваться между двумя вкладками;
   - интерфейс во второй вкладке должен обновляться автоматически.

### Что важно в решении

1. Не храните в `localStorage`:
   - пароли;
   - токены без необходимости;
   - чувствительные персональные данные.
2. Храните только то, что реально улучшает UX:
   - тему;
   - язык;
   - фильтры;
   - локальную корзину;
