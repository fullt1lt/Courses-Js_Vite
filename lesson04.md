...
## Параметры по умолчанию (default args) и современный синтаксис аргументов (`...args`)

Когда мы вызываем функцию, мы можем передать ей аргументы. Но в реальном коде часто бывает так, что:

- аргумент не передали вообще;
- аргумент иногда нужен, а иногда нет;
- количество аргументов заранее неизвестно.

Современный JavaScript решает эти ситуации двумя инструментами:

- **параметры по умолчанию** (default parameters)
- **rest-parameters** `...args` (современная замена старого `arguments`)

### Параметры по умолчанию (default parameters)

Если аргумент не передали, параметр будет равен `undefined`.
...
greet(); // undefined
```
Чтобы не получать `undefined`, можно задать значение по умолчанию прямо в объявлении функции:
...
Это работает не только со строками, но и с числами:
...
alert(power(3, 3)); // 27
alert(power(4));    // 16 (4 в квадрате)
```

### Rest-parameters: ...args

Иногда мы не знаем заранее, сколько аргументов передадут в функцию. Например, хотим посчитать сумму любого количества чисел:

- `sum(1, 2)`
- `sum(1, 2, 3, 4, 5)`

Для этого используется rest-синтаксис:
...
- ...args собирает все переданные аргументы в массив `args`
- дальше мы работаем с ним как с обычным массивом(В следующих лекциях мы подробно разберём работу с массивами)

### Rest + обычные параметры

Rest можно комбинировать с обычными параметрами. Например: первое значение отдельно, остальные — в `...rest`.

```js
function show(first, ...rest) {
  alert(`Первый: ${first}`);
  alert(`Остальные: ${rest}`);
}
...
// Остальные: 20,30,40
```
> Важно: rest-параметр должен быть последним в списке параметров.
