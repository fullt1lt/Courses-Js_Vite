...
До этого момента мы учились двум большим вещам: писать логику на `JavaScript` и управлять интерфейсом через `DOM`, события, формы и возможности браузера. Этого уже достаточно, чтобы делать небольшие интерактивные страницы.

Но как только проект начинает расти, появляется новая проблема: код становится не просто набором функций и обработчиков, а системой, где есть пользователи, товары, корзина, заказы, способы оплаты и правила работы с ними.

И главная задача в этот момент - не только *“как написать функцию”*, а *“как организовать код так, чтобы он оставался понятным и не ломался от изменений”*.
...
В этой лекции переводим часть логики `Storefront` из набора функций в доменные сущности. Цель - почувствовать, где классы реально упрощают код, а где они не нужны.

## Практика

1. Создайте класс `Product`:
   - поля: `id`, `title`, `price`, `category`, `stock`;
   - геттер `isInStock`;
   - метод `getLabel()`, который возвращает строку вида `Laptop - 24990 CZK`.

2. Создайте класс `CartItem`:
   - хранит `product` и `quantity`;
   - метод `increase()` увеличивает количество;
   - метод `decrease()` уменьшает количество, но не ниже `1`;
   - геттер `total` возвращает `product.price * quantity`.

3. Создайте класс `Cart` с приватным полем `#items`.
   Реализуйте:
   - `add(product)`
   - `remove(productId)`
   - `clear()`
   - `setQuantity(productId, qty)`
   - геттеры `items`, `itemsCount`, `total`.

4. Создайте класс `UserPreferences`:
   - приватные поля `#theme`, `#language`, `#favoriteIds`;
   - методы для изменения темы и языка;
   - методы `addFavorite(id)` / `removeFavorite(id)`;
   - метод `toJSON()`, который возвращает простой объект для сохранения в `localStorage`.

5. Создайте базовый класс `PaymentMethod` и один конкретный класс, например `CashPayment`.
   Базовый класс должен:
   - задавать контракт `pay(order, user)`;
   - кидать ошибку, если метод не переопределён.

## Домашняя работа

Соберите **доменную модель для `Storefront`**.
...
3. `AdminUser extends User`
   - поле `role = "admin"`;
   - метод `archiveProduct(product)`;
   - переопределённый `greet()` через `super.greet()`.

4. `Cart`
   - приватное поле `#items`;
   - добавление, удаление, очистка;
   - итоговая сумма и количество товаров.

5. `Order`
   - получает `user`, массив товаров и итоговую сумму;
...
7. Минимум два способа оплаты:
   - `BalancePayment`
   - `CashPayment` или `CardPayment`

### Сценарий проверки

Напишите код, который:

1. Создаёт 2-3 товара.
2. Создаёт пользователя с балансом.
3. Добавляет товары в корзину.
4. Создаёт заказ на основе корзины.
5. Оплачивает заказ через один способ оплаты.
6. Создаёт `AdminUser` и архивирует один товар.

### Критерии

1. Баланс нельзя менять напрямую.
2. `Cart` сам отвечает за свои данные, без внешнего редактирования массива.
3. `Order.checkout()` не знает конкретный тип оплаты, а вызывает `pay()` у переданного объекта.
4. В решении действительно видны:
   - инкапсуляция;
   - наследование;
   - полиморфизм;
   - `static`;
