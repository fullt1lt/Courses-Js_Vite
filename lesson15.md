# Лекция 15. Практический SPA-проект: роутинг, переводы и тема оформления
...
Если собрать эту лекцию в несколько главных мыслей:

- `SPA` загружает один основной документ и дальше меняет содержимое через `JavaScript`;
- переходы между разделами происходят без полной перезагрузки страницы;
- навигация обычно держится на клиентском роутинге;
- у `SPA` есть плюсы по ощущению “живого” интерфейса, но растёт ответственность frontend-кода;
- как только в приложении появляются страницы, язык, тема и состояние, проект уже начинает напоминать реальный продукт.

Если совсем коротко:

> **SPA - это не “один экран”, а один загруженный документ, внутри которого JavaScript сам управляет разделами приложения.**
...
`SPA` - это не значит, что в приложении вообще нет страниц как сущностей.

Наоборот, страницы есть:
...
Но они живут не как отдельные HTML-файлы, которые каждый раз полностью перезагружаются, а как разделы одного клиентского приложения.

### Плюсы и ограничения `SPA`

Почему этот подход популярен:

- интерфейс ощущается быстрее и живее;
- переходы можно делать без полной перезагрузки;
- проще держать общее состояние приложения;
- удобно строить более “приложенческий” UX.

Но есть и цена:

- больше логики переезжает на frontend;
- нужно самостоятельно управлять навигацией;
- нужно хранить состояние приложения;
- архитектура становится сложнее, чем у набора отдельных страниц.
...
В этой превращаем `Storefront` в настоящее маленькое `SPA`. Не создаём новый проект, а доводим уже знакомое приложение до цельного результата.
...
2. Реализуйте навигацию без полной перезагрузки страницы.
   Можно использовать `page.js` или собственный простой роутер.

3. Сделайте отдельные функции рендера для страниц:
   - `renderCatalogPage()`
   - `renderFavoritesPage()`
   - `renderCartPage()`
   - `renderSettingsPage()`
...
5. Добавьте пустые состояния:
   - если избранное пустое;
   - если корзина пустая;
   - если товар по `id` не найден.

## Домашняя работа

Финальный проект: **`Storefront SPA`**.

### Что должно быть в приложении

1. Главная страница или welcome-screen.
2. Страница каталога.
3. Страница избранного.
4. Страница корзины.
5. Страница настроек.
6. Страница деталей товара.
7. Страница `Not Found`.

### Обязательные требования

1. Навигация между страницами без полной перезагрузки.
2. Работают кнопки Назад/Вперёд браузера.
3. Можно открыть прямую ссылку на страницу товара.
4. Тема и язык сохраняются между переходами.
5. Корзина и избранное сохраняются между переходами.
6. На каждой странице есть понятные состояния:
   - loading;
   - empty;
   - error, если данные не удалось получить.

### Что будет сильным финалом

1. Один цельный проект, который развивался с `lesson07`.
2. Понятная структура файлов.
3. Реальные пользовательские сценарии:
   ...
   - возврат назад без потери состояния.
## Заключение
...
- объяснить, что такое `SPA`;
- отличать `SPA` от набора отдельных HTML-страниц;
- понимать, зачем приложению нужен клиентский роутинг;
- организовать структуру проекта под несколько страниц;
- подключить и настроить простую навигацию;
- реализовать переключение языка;
- хранить язык и тему в `localStorage`;
- собрать небольшой учебный `SPA` на базе уже созданного проекта.