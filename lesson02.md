# Лекция 2. Условия и логика в JavaScript: boolean, truthy/falsy, if/else, тернарный оператор, проверки ввода. Методы строк.
...
## Методы строк для валидации ввода: trim, регистр, поиск, длина

![](./images/methods_string.jpg)

До этого момента мы использовали строки в основном как *“контейнер”* для текста из `prompt()`. Но в реальной разработке строки почти всегда нужно обрабатывать:

- убрать лишние пробелы;
- сравнивать без учёта регистра (`Admin`, `ADMIN`, `admin`);
- проверить, что строка вообще *“похожа”* на то, что ожидаем (`email`, `номер`, `команда`);
- найти подстроку, проверить начало/конец.

Важно понимать: `prompt()` возвращает строку или `null`, поэтому методы строк мы применяем только после проверки на `null`.

На самом деле методов строк в JavaScript больше , вы можете их посмотреть [здесь](https://www.w3schools.com/js/js_string_methods.asp). Но для базовой валидации и обработки ввода достаточно знать эти 6:
...
Частая задача - проверка минимальной длины:
...
`trim()` удаляет пробелы и переносы строк в начале и конце строки. Это must-have для ввода.

```javascript
let raw = "   admin   ";
console.log(raw.trim()); // "admin"
```

Практическое правило:

- если проверяете “пустой ввод”, проверяйте `trim() === ""`.
...
### 3) toLowerCase() / toUpperCase(): сравнение без учёта регистра

Если нужно сравнить строки без учёта регистра, используйте `toLowerCase()` или `toUpperCase()`.
...
Тут важно обратить внимание на порядок: сначала `trim()`, затем `toLowerCase()`, чтобы корректно обрабатывать ввод с пробелами и разным регистром.

### 4) includes(): есть ли подстрока

`includes()` возвращает `true`, если строка содержит подстроку.

```javascript
console.log("hello world".includes("world")); // true
console.log("hello world".includes("World")); // false (регистр важен!)
```

Поэтому при “поиске” обычно тоже делают нормализацию регистра:
...
if (message === null) {
  ...
  if (text.includes("спам")) {
    alert("Сообщение содержит запрещённое слово");
  } else {
    alert("Ок");
  }
}
...
### 5) startsWith() / endsWith(): проверка начала и конца строки

Эти методы полезны, когда вы ожидаете конкретный формат.

```javascript
console.log("https://site.com".startsWith("https://")); // true
console.log("photo.jpg".endsWith(".jpg")); // true
```
...
> Важно: такая проверка не гарантирует “настоящий” email, но для первых задач достаточно, чтобы показать логику.

### 6) slice(): взять часть строки

`slice(start, end)` возвращает часть строки. Удобно для маскирования и демонстрации работы со строками.

```javascript
let card = "1234 5678 9012 3456";
let last4 = card.slice(-4);   // "3456"
alert(`**** **** **** ${last4}`);
```
...
Метод `replace()` полезен для замены запрещённых слов, форматирования и других задач. Он заменяет только первое вхождение. Для замены всех вхождений можно использовать регулярные выражения.
...
11) Запросите `answer` (“да/нет”). Если `Cancel` - “Отмена”. Если пусто - “Пустой ввод”. Иначе привести к нижнему регистру (`toLowerCase()`) и:

    - если `answer === "да"` или `answer === "yes"` → “OK”
    - если `answer === "нет"` или `answer === "no"` → “Denied”
    - иначе → “Не понял ответ”
12) Запросите `email`. Если `Cancel` - “Отмена”. Если пусто после `trim()` - “Пустой ввод”. Иначе вывести `true`, если строка содержит символ `@` (через `includes("@")`), иначе `false`.
13) Запросите `fileName`. Если `Cancel` - “Отмена”. Если пусто после `trim()` - “Пустой ввод”. Иначе вывести “OK”, если имя заканчивается на `.jpg` или `.png` (через `endsWith()`), иначе “Denied”.
14) Запросите строку `text` и строку `word`. Если любой `Cancel` - “Отмена”. Если любой пустой после `trim()` - “Пустой ввод”. Иначе вывести `true`, если `text` содержит `word` без учёта регистра (через `toLowerCase()` + `includes()`), иначе `false`.
15) Запросите строку `card`. Если `Cancel` - “Отмена”. Если пусто после `trim()` - “Пустой ввод”. Если длина меньше 4 - “Ошибка”. Иначе вывести последние 4 символа строки (через `slice(-4)`).
...
- если `Cancel` → `Отмена`
- если пустая строка после `trim()` → `Пустой ввод`
- иначе сравните логин без учёта регистра (через `toLowerCase()`):
  если логин равен `"admin"` → `Hello, admin`, иначе → `Unknown user`

12) Запросите `password`.

- если `Cancel` → `Отмена`
- если пустая строка после `trim()` → `Пустой ввод`
- иначе выведите `true`, если длина пароля (`length`) не меньше 8, иначе `false`

13) Запросите `email`.

- если `Cancel` → `Отмена`
- если пустая строка после `trim()` → `Пустой ввод`
- иначе вывести “OK”, если:
  - строка содержит `@` (через `includes("@")`)
  - строка не начинается с `@` (через `startsWith("@")`)
  - строка не заканчивается на `@` (через `endsWith("@")`)
    иначе вывести “Invalid email”

14) Запросите `fileName`.

- если `Cancel` → `Отмена`
- если пустая строка после `trim()` → `Пустой ввод`
- иначе вывести “OK”, если имя файла заканчивается на `.jpg` или `.png` (через `endsWith()`), иначе “Denied”

15) Запросите строку `text`.

- если `Cancel` → `Отмена`
- если пустая строка после `trim()` → `Пустой ввод`
- иначе вывести “Запрещено”, если строка содержит слово `"spam"` или `"реклама"` (через `toLowerCase()` + `includes()`), иначе “Ок”
