# Лекция 14. Tooling во frontend: npm, Vite, структура проекта, сборка и библиотеки

![](./images/tooling13.jpg)

## Введение

К этому моменту у нас уже есть не просто маленькие примеры, а довольно взрослый учебный frontend:

- `DOM`, события и формы;
- асинхронность;
- `fetch` и работа с `API`;
- модули и разбиение проекта по файлам.

И здесь появляется следующая естественная проблема:

> **сам код уже вырос, а среда разработки всё ещё слишком примитивная**

Пока проект маленький, можно жить в формате:

- `index.html`
- `style.css`
- `script.js`

Но как только проект становится больше, этого начинает не хватать. Нужна более удобная инфраструктура:

- запуск проекта;
- автоматическое обновление в браузере;
- подключение пакетов;
- сборка production-версии;
- нормальная структура рабочего проекта.

Именно для этого во frontend существует `tooling`.

## Кратко

Если сжать лекцию до нескольких тезисов:

- `tooling` - это не новый язык, а набор инструментов вокруг проекта;
- `Node.js` нужен frontend-разработчику не для интерфейса в браузере, а для экосистемы инструментов;
- `npm` помогает управлять пакетами и скриптами проекта;
- `Vite` даёт современный `dev`-сервер и production-сборку;
- хороший frontend-проект сегодня - это не просто набор файлов, а ещё и удобная среда разработки.

С практической точки зрения `Vite` особенно важен потому, что объединяет две ключевые вещи:

- быстрый `dev`-сервер с `HMR`;
- production-сборку проекта.
...
В Лекции 14 переносим `Storefront` в нормальную среду разработки. Цель - чтобы студент перестал работать как с набором файлов и начал работать как с проектом.

## Практика

1. Создайте новый проект на `Vite` (`vanilla`).
2. Перенесите текущий `Storefront` в `src/`.
3. Подключите глобальные стили через `import`.
4. Проверьте работу команд:
   - `npm run dev`
   - `npm run build`
   - `npm run preview`
5. Перенесите статические данные или картинки:
   - в `public/`, если это прямые файлы;
   - в `src/assets/`, если хотите импортировать их из кода.
6. Подключите одну библиотеку через `npm`.
   Хорошие варианты:
   - `dayjs` для форматирования даты обновления каталога;
   - `lodash.debounce` для поля поиска.

## Домашняя работа

Переведите проект в **`Storefront - Vite workflow`**.

### Обязательные требования

1. Проект создан через `Vite`.
2. Вся логика и модули находятся в `src/`.
3. Приложение запускается только через dev-server, а не через прямое открытие `index.html`.
4. Одна внешняя библиотека действительно используется в интерфейсе.
5. После `npm run build` проект собирается без ошибок.
6. После `npm run preview` собранная версия работает так же, как dev-версия.
...
2. Папку `public/` для мок-данных или изображений, если это нужно вашему приложению.
...
- объяснить, что такое `tooling` и зачем оно нужно;
- понимать роль `Node.js` во frontend-разработке;
- создать и прочитать `package.json`;
- запускать проект через `npm`-скрипты;
- создать проект на `Vite`;
- объяснить разницу между `dev`, `build` и `preview`;
- понимать, зачем нужен `HMR`;
- подключить библиотеку через `npm`.